	...
	R
		...
		.then(name => {
			...
			return cred.login().then(() => {
				FS.writeFile(global.AUTH_FILE, JSON.stringify(cred.getCookieJar()));
			});
		})
	...
	switch(data.type){
		...
		case 'w':
			...
			if(res.data == ""){
				...
				client.send('event', "whisper", res);
				client.send('event', "set-chat", { data: `/w ${data.target} ` });
			}
			break;
		default:
