...
// const KKTrans = require("node-ncc-es6/lib/translate");
...
module.exports = function(CoJer, Session){
	/*Session.prototype.getMsg = function(room, start, end){
		return this.sendCommand('GetMsg', {
			...
			err = new Error(`${body.retTitle}, ${body.retMsg}`);
			err.retCode = body.retCode;
			
			throw err;
		}).then(res => {
			let msgList = res.bdy.msgList;
			CoJer.log(msgList);
			let messages = msgList.map(msg => KKTrans.translateMessage(this,
				Object.assign({}, msg, {
					old: true,
					roomId: room.id,
					cafeId: room.cafe.id
